...
def _validate_positive(spec: Spec, text: str):
    ...
    # 片段之外不得有任何数字/NSW 符号(否则编辑脚本对该句不完备 → 标签噪声)
    leftover = []
    prev = 0
    for s, e, _ in spans:
        leftover.append(text[prev:s])
        prev = e
    leftover.append(text[prev:])
    rest = "".join(leftover)
...
def _validate_negative(spec: Spec, text: str):
    ...
    if not 5 <= len(text) <= 120:
        return None, "bad_length"
    if DIGIT_RE.search(text) or SYMBOL_RE.search(text):
    ...
    if spec.neg_type == "plain" and LATIN_RE.search(text):
...
async def main():
    ...
    specs = build_specs(args.n, args.seed, args.neg_ratio)
